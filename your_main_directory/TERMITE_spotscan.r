...
#			                  	TERMITE                                          	#
#								                                                      	    #
#			An R-Skript for fast reduction of LA_ICPMS data and                   #
...
path.rawData.samples	<- "Rawdata_spotscan/"   	      	# defining the directory of the raw-data-files
path.rawData.RefMat 	<- "ReferenceMaterial_spotscan/"  # defining the directory of the reference material
path.corrData.results	<- "Results/"		        # defining the directory for the overview-plots

sample.name	        	<- "your_sample_name_spotscan"	            # is printed in the file-names


##### before starting the script check the HelpValue Sections !! #####

##### general HelpValue Section #####
Mode.of.Scan                <- "spot.scan"        # spot.scan vs. line.scan 
...
column.IS		                <- 5				          # column of internal standard in the .asc-files
IS 		          	          <- 400003.81	        # [µg/g] internal standard
outlier.Test                <- "Y"                # "Y" vs. "N"
...
resolution		              <- "(LR)"		        	# Resolution "(LR)""(MR)""(HR)" (needed for ThermoFischer Element2 ICP-MS)
machine                     <- "Agilent"          # Element2 vs. Agilent

background.correction	      <- "median"		  	    # method for background.correction (mean <-> median)

##### HelpValue for Linescans Section #####
first.blankValue.linescan   <- 5			            # first Blank-Value used for background-correction
last.blankValue.linescan		<- 124				        # last  Blank-Value used for background-correction

first.sampleValue.linescan  <- 143				        # first Laser-on-sample-Value used for analysis
last.sampleValue.linescan	  <- 6890			          # last  Laser-on-sample-Value used for analysis

sample.speed                <- 5                  # sample scan speed in µm/s

##### HelpValues for spot scans and/or reference material Section #####
first.blankValue	          <- 5			            # first Blank-Value used for background-correction
last.blankValue		          <- 124				        # last  Blank-Value used for background-correction

first.sampleValue	          <- 180				        # first Laser-on-sample-Value used for analysis
last.sampleValue	          <- 440		        		# last  Laser-on-sample-Value used for analysis
