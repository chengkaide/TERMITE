...
**在线文档（GitHub Pages）：<https://chengkaide.github.io/TERMITE/>** —— 项目落地页，含快速开始与全部文档入口（启用方法见文末「发布为在线文档」）。
...
侧栏参数分 7 组折叠：导入数据、数据与仪器、行列与内标、积分窗口、离群检验、兼容与修正、高级。

### 无内标公式法

选矿物（白钨矿 / 锡石 / 锆石 / 磷灰石 / 云母 / 绿柱石 / 电气石）→ 运行，输出元素浓度、归一化因子与结果表。
...
## 发布为在线文档（GitHub Pages）

仓库里已放好落地页 `docs/index.html`，可直接发布成在线文档：

1. GitHub 仓库 → **Settings** → **Pages**
2. Source 选 **Deploy from a branch**
3. Branch 选 **master**、目录选 **/docs**，保存

几分钟后即可访问 `https://chengkaide.github.io/TERMITE/`，落地页会链接到入门手册与各文档。
`docs/.nojekyll` 已就位，页面按纯静态站点发布。
